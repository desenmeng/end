...
dal.update = function(data,callback){
    if(2==data.routes.length){
        ...
        mongo.update(collection,{'_id':new BSON.ObjectID(id)},{'$set':data.value}, data.options,callback);
    }
...
dal.set = function(data,callback){
    if(3==data.routes.length){
        ...
        mongo.update(collection,{'_id':new BSON.ObjectID(id)},{'$set':value}, {safe:true},callback);
    }
};
