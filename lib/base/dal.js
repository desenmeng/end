...
dal.insert = function(data,callback){
    if(1==data.routes.length){
        ...
        mongo.insert(collection,data.value, {safe:true},callback);
    }
};
dal.update = function(data,callback){
    if(2===data.routes.length){
        ...
        console.log(data);
        mongo.update(data.route,collection,{'_id':new BSON.ObjectID(id)},{'$set':data.value}, data.options,callback);
    }
};
dal.set = function(data,callback){
    if(3===data.routes.length){
        ...
        mongo.update(data.route,collection,{'_id':new BSON.ObjectID(id)},{'$set':value}, {safe:true},callback);
    }
};
dal.push = function(data,callback){
    if(3===data.routes.length){
        var collection = data.routes[0],
            ...
            value = {};
        value[data.routes[2]] = data.value;
        mongo.update(collection,{'_id':new BSON.ObjectID(id)},{'$push':value}, {safe:true},callback);
    }
};
